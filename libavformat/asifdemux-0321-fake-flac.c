...
{
  /* the real seek index offset should be the size of metadata blocks with the
   * offset in the frame blocks */
  int i;
...
static int flac_read_header(AVFormatContext* s)
{
  ...
  while (!avio_feof(s->pb) && !metadata_last)
  {
    ...
    if (metadata_type == FLAC_METADATA_TYPE_STREAMINFO)
    {
      ...
      samples =
          (AV_RB64(st->codecpar->extradata + 13) >> 24) & ((1ULL << 36) - 1);
    ...
    else if (metadata_type == FLAC_METADATA_TYPE_CUESHEET)
    {
      ...
      for (i = 0; i < chapters; i++)
      {
        ...
        if (ti <= 0)
          RETURN_ERROR(AVERROR_INVALIDDATA);
        offset += ti * 12;
        avpriv_new_chapter(s, track, st->time_base, start, AV_NOPTS_VALUE,
                           isrc);
      }
      av_freep(&buffer);
    }
    ...
    else
    {
      ...
      if (metadata_type == FLAC_METADATA_TYPE_VORBIS_COMMENT)
      {
        ...
        chmask = av_dict_get(s->metadata, "WAVEFORMATEXTENSIBLE_CHANNEL_MASK",
                             NULL, 0);
        if (chmask)
        {
          ...
          else
          {
            ...
            av_dict_set(&s->metadata, "WAVEFORMATEXTENSIBLE_CHANNEL_MASK", NULL,
                        0);
          }
        }
      }
...
{
  printf("------------ (asif) flac_probe() ------------\n");

  if (p->buf_size >= 4 && p->buf[0] == 'a' && p->buf[1] == 's' &&
      p->buf[2] == 'i' && p->buf[3] == 'f')
  ...
  if (p->buf_size >= 4 + 4 + 13)
  {
    ...
    if (type == FLAC_METADATA_TYPE_STREAMINFO && size == FLAC_STREAMINFO_SIZE &&
        min_block_size >= 16 && max_block_size >= min_block_size &&
...
static av_unused int64_t flac_read_timestamp(AVFormatContext* s,
                                             int stream_index, int64_t* ppos,
                                             int64_t pos_limit)
{
  AVPacket pkt;
  ...
  for (;;)
  {
    ...
    av_parser_parse2(parser, st->internal->avctx, &data, &size, pkt.data,
                     pkt.size, pkt.pts, pkt.dts, *ppos);
