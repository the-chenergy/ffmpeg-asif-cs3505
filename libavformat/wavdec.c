...
static int wav_probe(const AVProbeData* p)
{
  ...
  if (!memcmp(p->buf + 8, "WAVE", 4))
  {
    ...
    else if (!memcmp(p->buf, "RF64", 4) && !memcmp(p->buf + 12, "ds64", 4))
...
static int wav_parse_bext_tag(AVFormatContext* s, int64_t size)
{
  ...
  if (avio_rl16(s->pb) >= 1)
  {
    ...
    if (umid_mask)
    {
      ...
      if (umid_parts[4] == 0 && umid_parts[5] == 0 && umid_parts[6] == 0 &&
          umid_parts[7] == 0)
      {
        /* basic UMID */
...
static int wav_read_header(AVFormatContext* s)
{
  ...
  if (has_printed == 0)
  {
    av_log(NULL, AV_LOG_INFO,
           "*** CS 3505 Spring 2020:  Running code in wav_read_header in "
           "wavdec.c ***\n");
    av_log(NULL, AV_LOG_INFO,
           "*** CS 3505 Spring 2020:  Changed by Qianlang Chen and Autumrose "
           "Stubbs ***\n");
  ...
  for (;;)
  {
    ...
    switch (tag)
    {
      case MKTAG('f', 'm', 't', ' '):
        ...
        if (!got_xma2 && !got_fmt &&
            (ret = wav_parse_fmt_tag(s, size, &st)) < 0)
        {
          return ret;
        }
        else if (got_fmt)
      ...
      case MKTAG('X', 'M', 'A', '2'):
        ...
        if (!got_fmt && !got_xma2 &&
            (ret = wav_parse_xma2_tag(s, size, &st)) < 0)
        {
          return ret;
        }
        else if (got_xma2)
      ...
      case MKTAG('i', 'd', '3', ' '):
      {
        ...
        ff_id3v2_read_dict(pb, &s->internal->id3v2_meta, ID3v2_DEFAULT_MAGIC,
                           &id3v2_extra_meta);
        if (id3v2_extra_meta)
        {
...
break_loop:
  ...
  if (st->codecpar->bit_rate > 0 && data_size > 0 &&
      st->codecpar->sample_rate > 0 && sample_count > 0 &&
      st->codecpar->channels > 1 && sample_count % st->codecpar->channels == 0)
  {
    if (fabs(8.0 * data_size * st->codecpar->channels *
                 st->codecpar->sample_rate / sample_count /
  ...
  if (data_size > 0 && sample_count && st->codecpar->channels &&
      (data_size << 3) / sample_count / st->codecpar->channels >
          st->codecpar->bits_per_coded_sample + 1)
  {
  ...
  if (st->codecpar->codec_id == AV_CODEC_ID_G729 && sample_count &&
      (data_size << 3) > sample_count)
  {
    av_log(s, AV_LOG_WARNING, "ignoring wrong sample_count %" PRId64 "\n",
           sample_count);
    sample_count = 0;
  ...
  if (!sample_count || av_get_exact_bits_per_sample(st->codecpar->codec_id) > 0)
    if (st->codecpar->channels && data_size &&
...
static int wav_read_packet(AVFormatContext* s, AVPacket* pkt)
{
  ...
  smv_retry:
    ...
    if (audio_dts != AV_NOPTS_VALUE && video_dts != AV_NOPTS_VALUE)
    {
      ...
      wav->smv_last_stream =
          wav->smv_given_first
              ? av_compare_ts(video_dts, s->streams[1]->time_base, audio_dts,
                              s->streams[0]->time_base) > 0
              : 0;
                         ...
                         int64_t timestamp, int flags)
{
  ...
  if (wav->smv_data_ofs > 0)
  {
    ...
    if (stream_index == 0)
      smv_timestamp = av_rescale_q(timestamp, s->streams[0]->time_base,
  ...
  while (!avio_feof(pb))
  {
    ...
    else if (!memcmp(guid, ff_w64_guid_summarylist, 16))
    {
      ...
      for (i = 0; i < count; i++)
      {
        ...
        if (avio_feof(pb) || (cur = avio_tell(pb)) < 0 ||
            cur > end - 8 /* = tag + size */)
          break;
