...
#define APPLY_WEIGHT(weight, sample)                                           \
  ((sample) != (short)(sample) ? APPLY_WEIGHT_F(weight, sample)                \
...
static av_cold int wavpack_encode_init(AVCodecContext* avctx)
{
  ...
  if (!avctx->frame_size)
  {
    ...
    avctx->frame_size = block_samples;
           ...
           (avctx->frame_size < 128 || avctx->frame_size > WV_MAX_SAMPLES))
  {
  ...
  if (avctx->compression_level != FF_COMPRESSION_DEFAULT)
  {
    if (avctx->compression_level >= 3)
    {
      ...
      {
        s->num_branches = 4;
        s->extra_flags = EXTRA_TRY_DELTAS | EXTRA_ADJUST_DELTAS |
                         EXTRA_SORT_FIRST | EXTRA_SORT_LAST | EXTRA_BRANCHES;
      }
      else if (avctx->compression_level >= 7)
      {
        s->num_branches = 3;
        s->extra_flags = EXTRA_TRY_DELTAS | EXTRA_ADJUST_DELTAS |
                         EXTRA_SORT_FIRST | EXTRA_BRANCHES;
      }
      ...
      else if (avctx->compression_level >= 4)
      {
        s->num_branches = 1;
        s->extra_flags =
            EXTRA_TRY_DELTAS | EXTRA_ADJUST_DELTAS | EXTRA_BRANCHES;
      }
    }
...
static void shift_stereo(int32_t* left, int32_t* right, int nb_samples,
                         int shift)
{
  int i;
  for (i = 0; i < nb_samples; i++)
...
static int scan_float(WavPackEncodeContext* s, int32_t* samples_l,
                      int32_t* samples_r, int nb_samples)
{
...
static void scan_int23(WavPackEncodeContext* s, int32_t* samples_l,
                       int32_t* samples_r, int nb_samples)
{
...
static int scan_int32(WavPackEncodeContext* s, int32_t* samples_l,
                      int32_t* samples_r, int nb_samples)
{
  uint32_t magdata = 0, ordata = 0, xordata = 0, anddata = ~0;
  uint32_t crc = 0xffffffffu;
                        ...
                        int nb_samples, struct Decorr* dpp, int dir)
{
  ...
  if (dpp->value > MAX_TERM)
  {
    while (nb_samples--)
    {
      ...
      sam_A = ((3 - (dpp->value & 1)) * dpp->samplesA[0] - dpp->samplesA[1]) >>
              !(dpp->value & 1);
...
static void decorr_mono_buffer(int32_t* samples, int32_t* outsamples,
                               int nb_samples, struct Decorr* dpp, int tindex)
{
...
static void analyze_mono(WavPackEncodeContext* s, int32_t* samples,
                         int do_samples)
{
  ...
  for (i = 0; i < info.nterms && info.dps[i].value; i++)
    decorr_mono(s->sampleptrs[i][0], s->sampleptrs[i + 1][0], s->block_samples,
                info.dps + i, 1);

  info.best_bits =
      log2mono(s->sampleptrs[info.nterms][0], s->block_samples, 0) * 1;
  memcpy(s->sampleptrs[info.nterms + 1][0], s->sampleptrs[i][0],
         s->block_samples * 4);
  ...
  if (s->extra_flags & EXTRA_TRY_DELTAS)
  {
    ...
    if ((s->extra_flags & EXTRA_ADJUST_DELTAS) && s->decorr_passes[0].value)
      s->delta_decay =
          (float)((s->delta_decay * 2.0 + s->decorr_passes[0].delta) / 3.0);
    else
      s->delta_decay = 2.0;
  }
...
static void scan_word(WavPackEncodeContext* s, WvChannel* c, int32_t* samples,
                      int nb_samples, int dir)
{
  if (dir < 0)
...
static int wv_mono(WavPackEncodeContext* s, int32_t* samples, int no_history,
                   int do_samples)
{
  ...
  for (pi = 0; pi < s->num_passes;)
  {
    ...
    else
    {
      ...
      {
        s->mask_decorr =
            s->mask_decorr ? ((s->mask_decorr << 1) & (s->num_decorrs - 1)) : 1;
        continue;
      }
    }
    ...
    if (size < best_size)
    {
      ...
      memcpy(s->decorr_passes, save_decorr_passes,
             sizeof(struct Decorr) * MAX_TERMS);
      s->num_terms = nterms;
      s->best_decorr = c;
      best_size = size;
    }
...
static void decorr_stereo(int32_t* in_left, int32_t* in_right,
                          int32_t* out_left, int32_t* out_right, int nb_samples,
                          struct Decorr* dpp, int dir)
{
  int m = 0, i;
  ...
  switch (dpp->value)
  {
    case 2:
      while (nb_samples--)
      {
        ...
        out_left[0] = tmp =
            (dpp->samplesA[1] = in_left[0]) - APPLY_WEIGHT(dpp->weightA, sam);
        ...
        out_right[0] = tmp =
            (dpp->samplesB[1] = in_right[0]) - APPLY_WEIGHT(dpp->weightB, sam);
    ...
    case 17:
      while (nb_samples--)
      {
        ...
        dpp->samplesA[1] = dpp->samplesA[0];
        out_left[0] = tmp =
            (dpp->samplesA[0] = in_left[0]) - APPLY_WEIGHT(dpp->weightA, sam);
        UPDATE_WEIGHT(dpp->weightA, dpp->delta, sam, tmp);
        dpp->sumA += dpp->weightA;
        ...
        out_right[0] = tmp =
            (dpp->samplesB[0] = in_right[0]) - APPLY_WEIGHT(dpp->weightB, sam);
        UPDATE_WEIGHT(dpp->weightB, dpp->delta, sam, tmp);
        dpp->sumB += dpp->weightB;
    ...
    default:
    {
      ...
      while (nb_samples--)
      {
        ...
        sam = dpp->samplesA[m];
        out_left[0] = tmp =
            (dpp->samplesA[k] = in_left[0]) - APPLY_WEIGHT(dpp->weightA, sam);
        UPDATE_WEIGHT(dpp->weightA, dpp->delta, sam, tmp);
        dpp->sumA += dpp->weightA;

        sam = dpp->samplesB[m];
        out_right[0] = tmp =
            (dpp->samplesB[k] = in_right[0]) - APPLY_WEIGHT(dpp->weightB, sam);
        UPDATE_WEIGHT(dpp->weightB, dpp->delta, sam, tmp);
        dpp->sumB += dpp->weightB;
    ...
    case -1:
      while (nb_samples--)
      {
        ...
        out_left[0] = tmp =
            (sam_B = in_left[0]) - APPLY_WEIGHT(dpp->weightA, sam_A);
        UPDATE_WEIGHT_CLIP(dpp->weightA, dpp->delta, sam_A, tmp);
        dpp->sumA += dpp->weightA;

        out_right[0] = tmp = (dpp->samplesA[0] = in_right[0]) -
                             APPLY_WEIGHT(dpp->weightB, sam_B);
    ...
    case -2:
      while (nb_samples--)
      {
        ...
        out_right[0] = tmp =
            (sam_A = in_right[0]) - APPLY_WEIGHT(dpp->weightB, sam_B);
        UPDATE_WEIGHT_CLIP(dpp->weightB, dpp->delta, sam_B, tmp);
        dpp->sumB += dpp->weightB;

        out_left[0] = tmp =
            (dpp->samplesB[0] = in_left[0]) - APPLY_WEIGHT(dpp->weightA, sam_A);
        UPDATE_WEIGHT_CLIP(dpp->weightA, dpp->delta, sam_A, tmp);
        dpp->sumA += dpp->weightA;
  ...
  switch (dpp->value)
  {
    case 2:
      for (i = 0; i < nb_samples; i++)
      {
        ...
        dpp->samplesA[0] = dpp->samplesA[1];
        out_left[i] = tmp =
            (dpp->samplesA[1] = in_left[i]) - APPLY_WEIGHT_I(dpp->weightA, sam);
        UPDATE_WEIGHT(dpp->weightA, dpp->delta, sam, tmp);

        sam = dpp->samplesB[0];
        dpp->samplesB[0] = dpp->samplesB[1];
        out_right[i] = tmp = (dpp->samplesB[1] = in_right[i]) -
                             APPLY_WEIGHT_I(dpp->weightB, sam);
        UPDATE_WEIGHT(dpp->weightB, dpp->delta, sam, tmp);
      }
      break;
    case 17:
      for (i = 0; i < nb_samples; i++)
      {
        ...
        dpp->samplesA[1] = dpp->samplesA[0];
        out_left[i] = tmp =
            (dpp->samplesA[0] = in_left[i]) - APPLY_WEIGHT_I(dpp->weightA, sam);
        UPDATE_WEIGHT(dpp->weightA, dpp->delta, sam, tmp);

        sam = 2 * dpp->samplesB[0] - dpp->samplesB[1];
        dpp->samplesB[1] = dpp->samplesB[0];
        out_right[i] = tmp = (dpp->samplesB[0] = in_right[i]) -
                             APPLY_WEIGHT_I(dpp->weightB, sam);
        UPDATE_WEIGHT(dpp->weightB, dpp->delta, sam, tmp);
      }
      break;
    ...
    default:
    {
      ...
      for (i = 0; i < nb_samples; i++)
      {
        ...
        sam = dpp->samplesA[m];
        out_left[i] = tmp =
            (dpp->samplesA[k] = in_left[i]) - APPLY_WEIGHT_I(dpp->weightA, sam);
        UPDATE_WEIGHT(dpp->weightA, dpp->delta, sam, tmp);

        sam = dpp->samplesB[m];
        out_right[i] = tmp = (dpp->samplesB[k] = in_right[i]) -
                             APPLY_WEIGHT_I(dpp->weightB, sam);
        UPDATE_WEIGHT(dpp->weightB, dpp->delta, sam, tmp);
    ...
    case -1:
      for (i = 0; i < nb_samples; i++)
      {
        ...
        sam_A = dpp->samplesA[0];
        out_left[i] = tmp =
            (sam_B = in_left[i]) - APPLY_WEIGHT_I(dpp->weightA, sam_A);
        UPDATE_WEIGHT_CLIP(dpp->weightA, dpp->delta, sam_A, tmp);

        out_right[i] = tmp = (dpp->samplesA[0] = in_right[i]) -
                             APPLY_WEIGHT_I(dpp->weightB, sam_B);
        UPDATE_WEIGHT_CLIP(dpp->weightB, dpp->delta, sam_B, tmp);
      }
      break;
    case -2:
      for (i = 0; i < nb_samples; i++)
      {
        ...
        sam_B = dpp->samplesB[0];
        out_right[i] = tmp =
            (sam_A = in_right[i]) - APPLY_WEIGHT_I(dpp->weightB, sam_B);
        UPDATE_WEIGHT_CLIP(dpp->weightB, dpp->delta, sam_B, tmp);

        out_left[i] = tmp = (dpp->samplesB[0] = in_left[i]) -
                            APPLY_WEIGHT_I(dpp->weightA, sam_A);
        UPDATE_WEIGHT_CLIP(dpp->weightA, dpp->delta, sam_A, tmp);
      }
      break;
    case -3:
...
static void decorr_stereo_buffer(WavPackExtraInfo* info, int32_t* in_left,
                                 int32_t* in_right, int32_t* out_left,
                                 int32_t* out_right, int nb_samples, int tindex)
{
  ...
  dp.value = term;
  dp.delta = pre_delta;
  decorr_stereo(in_left, in_right, out_left, out_right, FFMIN(2048, nb_samples),
                &dp, -1);
  dp.delta = delta;
...
static void sort_stereo(WavPackEncodeContext* s, WavPackExtraInfo* info)
{
  ...
  while (reversed)
  {
    ...
    for (ri = 0; ri < info->nterms && s->decorr_passes[ri].value; ri++)
    {
      ...
      if (s->decorr_passes[ri].value == s->decorr_passes[ri + 1].value)
      {
        decorr_stereo_buffer(info, s->sampleptrs[ri][0], s->sampleptrs[ri][1],
      ...
      for (i = ri; i < info->nterms && s->decorr_passes[i].value; i++)
        decorr_stereo_buffer(info, s->sampleptrs[i][0], s->sampleptrs[i][1],
...
static void analyze_stereo(WavPackEncodeContext* s, int32_t* in_left,
                           int32_t* in_right, int do_samples)
{
  WavPackExtraInfo info;
  int i;
  ...
  info.best_bits =
      log2stereo(s->sampleptrs[info.nterms][0], s->sampleptrs[info.nterms][1],
                 s->block_samples, 0);
...
static int wv_stereo(WavPackEncodeContext* s, int32_t* samples_l,
                     int32_t* samples_r, int no_history, int do_samples)
{
  struct Decorr temp_decorr_pass, save_decorr_passes[MAX_TERMS] = {{0}};
    ...
    while (1)
    {
      ...
      for (j = 0; j < nterms; j++)
      {
        ...
        if (((s->flags & MAG_MASK) >> MAG_LSB) >= 16)
          ...
          decorr_stereo_quick(
              s->temp_buffer[j & 1][0], s->temp_buffer[j & 1][1],
              s->temp_buffer[~j & 1][0], s->temp_buffer[~j & 1][1], nb_samples,
              &temp_decorr_pass);
      }
...
static void wavpack_encode_sample(WavPackEncodeContext* s, WvChannel* c,
                                  int32_t sample)
{
  WavPackWords* w = &s->w;
...
static void pack_int32(WavPackEncodeContext* s, int32_t* samples_l,
                       int32_t* samples_r, int nb_samples)
{
...
static void pack_float(WavPackEncodeContext* s, int32_t* samples_l,
                       int32_t* samples_r, int nb_samples)
{
  int i;
...
static void decorr_stereo_pass2(struct Decorr* dpp, int32_t* samples_l,
                                int32_t* samples_r, int nb_samples)
{
  int i, m, k;
  ...
  switch (dpp->value)
  {
    case 17:
      for (i = 0; i < nb_samples; i++)
      {
        ...
        dpp->samplesA[1] = dpp->samplesA[0];
        samples_l[i] = tmp =
            (dpp->samplesA[0] = samples_l[i]) - APPLY_WEIGHT(dpp->weightA, sam);
        UPDATE_WEIGHT(dpp->weightA, dpp->delta, sam, tmp);

        sam = 2 * dpp->samplesB[0] - dpp->samplesB[1];
        dpp->samplesB[1] = dpp->samplesB[0];
        samples_r[i] = tmp =
            (dpp->samplesB[0] = samples_r[i]) - APPLY_WEIGHT(dpp->weightB, sam);
        UPDATE_WEIGHT(dpp->weightB, dpp->delta, sam, tmp);
      }
      break;
    case 18:
    ...
    default:
      for (m = 0, k = dpp->value & (MAX_TERM - 1), i = 0; i < nb_samples; i++)
      {
        ...
        sam = dpp->samplesA[m];
        samples_l[i] = tmp =
            (dpp->samplesA[k] = samples_l[i]) - APPLY_WEIGHT(dpp->weightA, sam);
        UPDATE_WEIGHT(dpp->weightA, dpp->delta, sam, tmp);

        sam = dpp->samplesB[m];
        samples_r[i] = tmp =
            (dpp->samplesB[k] = samples_r[i]) - APPLY_WEIGHT(dpp->weightB, sam);
        UPDATE_WEIGHT(dpp->weightB, dpp->delta, sam, tmp);
    ...
    case -1:
      for (i = 0; i < nb_samples; i++)
      {
        ...
        sam_A = dpp->samplesA[0];
        samples_l[i] = tmp =
            (sam_B = samples_l[i]) - APPLY_WEIGHT(dpp->weightA, sam_A);
        UPDATE_WEIGHT_CLIP(dpp->weightA, dpp->delta, sam_A, tmp);

        samples_r[i] = tmp = (dpp->samplesA[0] = samples_r[i]) -
                             APPLY_WEIGHT(dpp->weightB, sam_B);
        UPDATE_WEIGHT_CLIP(dpp->weightB, dpp->delta, sam_B, tmp);
      }
      break;
    case -2:
      for (i = 0; i < nb_samples; i++)
      {
        ...
        sam_B = dpp->samplesB[0];
        samples_r[i] = tmp =
            (sam_A = samples_r[i]) - APPLY_WEIGHT(dpp->weightB, sam_B);
        UPDATE_WEIGHT_CLIP(dpp->weightB, dpp->delta, sam_B, tmp);

        samples_l[i] = tmp = (dpp->samplesB[0] = samples_l[i]) -
                             APPLY_WEIGHT(dpp->weightA, sam_A);
        UPDATE_WEIGHT_CLIP(dpp->weightA, dpp->delta, sam_A, tmp);
      }
      break;
    case -3:
...
#define update_weight_clip_d2(weight, delta, source, result)                   \
  ...
  {                                                                            \
    ...
    if ((weight = (weight ^ s) + (2 - s)) > 1024)                              \
      weight = 1024;                                                           \
    weight = (weight ^ s) - s;                                                 \
  }

static void decorr_stereo_pass_id2(struct Decorr* dpp, int32_t* samples_l,
                                   int32_t* samples_r, int nb_samples)
{
  int i, m, k;
  ...
  switch (dpp->value)
  {
    case 17:
      for (i = 0; i < nb_samples; i++)
      {
        ...
        sam = 2 * dpp->samplesA[0] - dpp->samplesA[1];
        dpp->samplesA[1] = dpp->samplesA[0];
        samples_l[i] = tmp = (dpp->samplesA[0] = samples_l[i]) -
                             APPLY_WEIGHT_I(dpp->weightA, sam);
        update_weight_d2(dpp->weightA, dpp->delta, sam, tmp);

        sam = 2 * dpp->samplesB[0] - dpp->samplesB[1];
        dpp->samplesB[1] = dpp->samplesB[0];
        samples_r[i] = tmp = (dpp->samplesB[0] = samples_r[i]) -
                             APPLY_WEIGHT_I(dpp->weightB, sam);
    ...
    case -1:
      for (i = 0; i < nb_samples; i++)
      {
        ...
        sam_A = dpp->samplesA[0];
        samples_l[i] = tmp =
            (sam_B = samples_l[i]) - APPLY_WEIGHT_I(dpp->weightA, sam_A);
    ...
    case -2:
      for (i = 0; i < nb_samples; i++)
      {
        ...
        sam_B = dpp->samplesB[0];
        samples_r[i] = tmp =
            (sam_A = samples_r[i]) - APPLY_WEIGHT_I(dpp->weightB, sam_B);
        update_weight_clip_d2(dpp->weightB, dpp->delta, sam_B, tmp);
...
static int wavpack_encode_block(WavPackEncodeContext* s, int32_t* samples_l,
                                int32_t* samples_r, uint8_t* out, int out_size)
{
  ...
  {
    av_fast_padded_malloc(&s->orig_l, &s->orig_l_size,
                          sizeof(int32_t) * nb_samples);
    memcpy(s->orig_l, samples_l, sizeof(int32_t) * nb_samples);
    if (!(s->flags & WV_MONO_DATA))
    {
      av_fast_padded_malloc(&s->orig_r, &s->orig_r_size,
                            sizeof(int32_t) * nb_samples);
  ...
  for (i = 0; i < s->num_terms; i++)
  {
    ...
    bytestream2_put_byte(&pb, ((dpp->value + 5) & 0x1f) |
                                  ((dpp->delta << 5) & 0xe0));
  }
  if (s->num_terms & 1)
    bytestream2_put_byte(&pb, 0);
...
#define WRITE_CHAN_ENTROPY(chan)                                               \
  ...
  put_metadata_block(&pb, WP_ID_ENTROPY,
                     6 * (1 + (!(s->flags & WV_MONO_DATA))));
...
static void fill_buffer(WavPackEncodeContext* s, const int8_t* src,
                        int32_t* dst, int nb_samples)
{
  int i;
                                ...
                                const AVFrame* frame, int* got_packet_ptr)
{
  ...
  for (s->ch_offset = 0; s->ch_offset < avctx->channels;)
  {
    ...
    fill_buffer(s, frame->extended_data[s->ch_offset], s->samples[0],
                s->block_samples);
    ...
    else
    {
      ...
      fill_buffer(s, frame->extended_data[s->ch_offset + 1], s->samples[1],
                  s->block_samples);
    }

    s->flags += (1 << MAG_LSB) * ((s->flags & 3) * 8 + 7);

    if ((ret = wavpack_encode_block(s, s->samples[0], s->samples[1], buf,
                                    buf_size)) < 0)
      return ret;
...
AVCodec ff_wavpack_encoder = {
    ...
    .sample_fmts =
        (const enum AVSampleFormat[]){AV_SAMPLE_FMT_U8P, AV_SAMPLE_FMT_S16P,
