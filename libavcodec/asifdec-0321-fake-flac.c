...
static av_cold int flac_decode_init(AVCodecContext* avctx)
{
  ...
  if (ret < 0)
    return ret;
  flac_set_bps(s);
  ff_flacdsp_init(&s->dsp, avctx->sample_fmt, s->flac_stream_info.channels,
                  s->flac_stream_info.bps);
  s->got_streaminfo = 1;
...
static int allocate_buffers(FLACContext* s)
{
  ...
  ret = av_samples_fill_arrays((uint8_t**)s->decoded, NULL, s->decoded_buffer,
...
static int parse_streaminfo(FLACContext* s, const uint8_t* buf, int buf_size)
{
  ...
  if (ret < 0)
    return ret;
  flac_set_bps(s);
  ff_flacdsp_init(&s->dsp, s->avctx->sample_fmt, s->flac_stream_info.channels,
                  s->flac_stream_info.bps);
  s->got_streaminfo = 1;
                               ...
                               int bps)
{
  ...
  else
  {
    ...
    if (s->flac_stream_info.bps <= 16)
      lpc_analyze_remodulate(decoded, coeffs, pred_order, qlevel, s->blocksize,
                             bps);
  }
...
static inline int decode_subframe(FLACContext* s, int channel)
{
  ...
  {
    if (s->ch_mode == FLAC_CHMODE_LEFT_SIDE ||
        s->ch_mode == FLAC_CHMODE_MID_SIDE)
      bps++;
  }
  ...
  if (get_bits1(&s->gb))
  {
    ...
    if (left <= 0 || (left < bps && !show_bits_long(&s->gb, left)) ||
...
static int decode_frame(FLACContext* s)
{
  ...
  if (s->flac_stream_info.channels &&
      fi.channels != s->flac_stream_info.channels && s->got_streaminfo)
  {
                             ...
                             int* got_frame_ptr, AVPacket* avpkt)
{
  ...
  *got_frame_ptr = 0;
  ...
  {
    s->flac_stream_info.max_framesize = ff_flac_get_max_frame_size(
        s->flac_stream_info.max_blocksize ? s->flac_stream_info.max_blocksize
                                          : FLAC_MAX_BLOCKSIZE,
  ...
  if ((s->avctx->err_recognition & (AV_EF_CRCCHECK | AV_EF_COMPLIANT)) &&
      av_crc(av_crc_get_table(AV_CRC_16_ANSI), 0, buf, bytes_read))
  {
  ...
  s->dsp.decorrelate[s->ch_mode](frame->data, s->decoded,
                                 s->flac_stream_info.channels, s->blocksize,
                                 s->sample_shift);
...
static const AVOption options[] = {
    {"use_buggy_lpc",
     ...
     offsetof(FLACContext, buggy_lpc),
     AV_OPT_TYPE_BOOL,
     {.i64 = 0},
     0,
     1,
...
AVCodec ff_asif_decoder = {
    ...
    .sample_fmts =
        (const enum AVSampleFormat[]){AV_SAMPLE_FMT_S16, AV_SAMPLE_FMT_S16P,
