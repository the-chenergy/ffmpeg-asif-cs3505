...
static int bmp_decode_frame(AVCodecContext* avctx, void* data, int* got_frame,
                            AVPacket* avpkt)
{
  ...
  if (has_printed == 0)
  {
    av_log(NULL, AV_LOG_INFO,
           "*** CS 3505 Spring 2020:  Running code in bmp_decode_frame in "
           "bmp.c ***\n");
    av_log(NULL, AV_LOG_INFO,
  ...
  if (bytestream_get_byte(&buf) != 'B' || bytestream_get_byte(&buf) != 'M')
  {
  ...
  /* sometimes file size is set to some headers size, set a real size in that
   * case */
  ...
  ret =
      ff_set_dimensions(avctx, width, height > 0 ? height : -(unsigned)height);
  if (ret < 0)
  {
  ...
  switch (depth)
  {
    case 32:
      if (comp == BMP_BITFIELDS)
      {
        if (rgb[0] == 0xFF000000 && rgb[1] == 0x00FF0000 &&
            rgb[2] == 0x0000FF00)
          avctx->pix_fmt = alpha ? AV_PIX_FMT_ABGR : AV_PIX_FMT_0BGR;
        else if (rgb[0] == 0x00FF0000 && rgb[1] == 0x0000FF00 &&
                 rgb[2] == 0x000000FF)
          avctx->pix_fmt = alpha ? AV_PIX_FMT_BGRA : AV_PIX_FMT_BGR0;
        else if (rgb[0] == 0x0000FF00 && rgb[1] == 0x00FF0000 &&
                 rgb[2] == 0xFF000000)
          avctx->pix_fmt = alpha ? AV_PIX_FMT_ARGB : AV_PIX_FMT_0RGB;
        else if (rgb[0] == 0x000000FF && rgb[1] == 0x0000FF00 &&
                 rgb[2] == 0x00FF0000)
