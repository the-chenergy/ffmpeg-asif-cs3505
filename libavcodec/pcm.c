                            ...
                            const AVFrame* frame, int* got_packet_ptr)
{
  ...
  if ((ret = ff_alloc_packet2(avctx, avpkt, n * sample_size, n * sample_size)) <
      0)
    return ret;
  dst = avpkt->data;
...
static av_cold int pcm_decode_init(AVCodecContext* avctx)
{
  ...
  if (has_printed == 0)
  {
    av_log(NULL, AV_LOG_INFO,
           "*** CS 3505 Spring 2020:  Running code in pcm_decode_init in "
           "pcm.c ***\n");
    av_log(NULL, AV_LOG_INFO,
                            ...
                            int* got_frame_ptr, AVPacket* avpkt)
{
  ...
  if (n && buf_size % n)
  {
    if (buf_size < n)
    {
      av_log(avctx, AV_LOG_ERROR,
             "Invalid PCM packet, data has size %d but at least a size of %d "
             "was expected\n",
             buf_size, n);
      return AVERROR_INVALIDDATA;
    }
  ...
  switch (avctx->codec_id)
  {
    ...
    case AV_CODEC_ID_PCM_S24DAUD:
      for (; n > 0; n--)
      {
        ...
        AV_WN16A(samples,
                 ff_reverse[(v >> 8) & 0xff] + (ff_reverse[v & 0xff] << 8));
        samples += 2;
      }
      break;
    case AV_CODEC_ID_PCM_U16LE:
    ...
    case AV_CODEC_ID_PCM_LXF:
    {
      ...
      for (c = 0; c < avctx->channels; c++)
      {
        ...
        for (i = 0; i < n; i++)
        {
          ...
          *dst_int32_t++ = ((uint32_t)src[2] << 28) | (src[1] << 20) |
                           (src[0] << 12) | ((src[2] & 0x0F) << 8) | src[1];
          // extract high 20 bits and expand to 32 bits
          *dst_int32_t++ = (src[4] << 24) | (src[3] << 16) |
                           ((src[2] & 0xF0) << 8) | (src[4] << 4) |
                           (src[3] >> 4);
          src += 5;
        }
      }
      ...
      avctx->codec_id == AV_CODEC_ID_PCM_F24LE)
  {
    s->fdsp->vector_fmul_scalar(
        (float*)frame->extended_data[0], (const float*)frame->extended_data[0],
...
PCM_DECODER(PCM_F16LE, AV_SAMPLE_FMT_FLT, pcm_f16le,
            "PCM 16.8 floating point little-endian");
PCM_DECODER(PCM_F24LE, AV_SAMPLE_FMT_FLT, pcm_f24le,
